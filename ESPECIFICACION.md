...
### Historia (texto exacto y detalles)
"La Cofradía del Santísimo Cristo Resucitado de Montoro fue fundada en 1966 gracias a la iniciativa de un grupo de devotos locales, no habiendo un único fundador, sino varios colaboradores de la localidad. Nació con el propósito de rendir culto a Jesucristo Resucitado, centro de la fe cristiana. Desde su fundación participa activamente en la vida religiosa y cofrade de la localidad, siendo la encargada de culminar la Semana Santa de Montoro con su salida procesional en la mañana del Domingo de Resurrección. A lo largo de los años ha mantenido su compromiso con la devoción, la formación y la conservación de las tradiciones cofrades de la ciudad."

En la sección de historia se incluirá una subsección titulada **"Símbolos e Iconografía"** que detallará el significado del escudo: el Calvario, la Cruz gloriosa, la palma de victoria de Cristo sobre la muerte, la bandera PAX, las iniciales marianas y la ornamentación barroca roja.

### Escudo oficial y formato
- **Formato del Escudo:** Se representará de forma nativa mediante un dibujo vectorial SVG embebido directamente en el HTML (no mediante imágenes PNG o JPG externas) para asegurar carga instantánea y resolución perfecta.
...
- **Vocales:** Vocal 1, Vocal 2, Vocal 3, Vocal 4 (Nombres de ejemplo temporales para maquetación)
...
- **Email:** secretaria@cristoresucitadomontoro.es (ficticio para la demo)
- **Teléfono:** 957 16 00 12 (ficticio para la demo)
- **Dirección postal:** Montoro, Córdoba
- **Sede Canónica:** Ermita de Nuestra Señora de Gracia. Se incluirá un mapa interactivo de Google Maps embebido (iframe) en la sección de contacto y de historia.
- **Redes sociales:** Sin perfiles activos. Mostrar iconos de redes sociales (FB, Instagram, X) en el menú lateral y en el footer, enlazando a "#".
- **Buzón de sugerencias:** No es necesario para esta demostración.
...
- **Menú de navegación:** 12 enlaces en horizontal (desktop) / menú hamburguesa en panel lateral (drawer) deslizando desde la derecha que ocupa aproximadamente el 80% de la pantalla (móvil), permitiendo ver el resto con un overlay oscuro de fondo.
- **Icono de carrito** con contador de productos (badge circular rojo)
- El menú móvil debe cerrarse al hacer clic en un enlace, al hacer swipe a la derecha, al pulsar Escape, o al hacer clic en el overlay oscuro.
...
- Título "Escudo" con divisor dorado
- Layout 2 columnas en desktop:
  - Columna izquierda: descripción del escudo e historia de sus símbolos (representado de manera interactiva) + paleta de colores corporativos con 3 círculos (rojo, blanco, marfil) y sus códigos hex.
  - Columna derecha: Escudo de la Cofradía representado de forma vectorial nativa mediante SVG interactivo con efecto 3D en hover (`perspective` + `rotateY`).
- En móvil: imagen SVG arriba, texto debajo.
...
- Layout 2 columnas en desktop:
  - Columna izquierda: descripción detallada del Solemne Triduo y actividades de la Cofradía. Al ser la información provista reducida, se rellenará con un texto explicativo coherente y secciones en "Lorem Ipsum" en español para la demostración.
  - Columna derecha: componente de calendario/eventos interactivo y completo con:
    - Pestañas "Calendario" / "Próximos eventos".
    - Vista calendario: soporta la navegación por todos los meses y años, cargando por defecto el mes y año actuales. Muestra los días destacados (ejemplo: 10 al 13 de abril de 2026 para la Semana Santa) con un puntito rojo decorativo. Al hacer clic en un día con evento, se abre un modal flotante con la información detallada del evento.
    - Vista lista: muestra de forma clara los 4 eventos del Triduo (días 1, 2, 3) + Domingo de Resurrección.
    - Flechas para cambiar de mes anterior/siguiente.
    - Cabecera del calendario en rojo.
    - Botón de exportación o suscripción al calendario para añadir a Google Calendar o iCal.
...
- Layout 2 columnas en desktop:
  ...
  - Debajo de los stats: tarjeta de itinerario con nota de actualización anual y listado de calles (usar texto ficticio legible).
  - Columna derecha: mapa de recorrido integrado mediante el iframe de Google Maps apuntando a Montoro, Córdoba (Ermita de la Virgen de Gracia o centro histórico).
- **Animación de contadores:** Al entrar en esta página, los números hacen animación de conteo con alta precisión matemática desde 0 hasta su valor. Solo se ejecuta la primera vez que se visita la página.
